        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(Colorate.Horizontal(Colors.purple_to_red, f'TikTok Viewbot by TA TOOL^| success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}'))
                    success += 1
                    _lock.release()
            except:
...
if __name__ == "__main__":
    ...
    set_title("HN TOOL")
    txt = """\n\n
    TikTok Viewbot by Sakib Hassan \n"""
    print(
        Colorate.Vertical(
